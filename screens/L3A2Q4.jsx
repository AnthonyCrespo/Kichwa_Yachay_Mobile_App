...
const L3A2Q4 = ({navigation}) => {
  ...
  const result = () => {
  if (verifyConcatenation(receivingItemList,'Maykanwiwatapukakan')){
      alert('Respuesta:\n'+'Correcto');
      resetLists();
 ...
 */}             
                {receivingItemList.map((item, index) => (
                  <View style={{ width: '25%' }} key={index}>
                    <ReceivingZoneUIComponent item={item} index={index} />
                  </View>
                ))}
              </View>
              ...
              style={styles.buttonContainer}
                  onPress={() => {
                    if (currentButtonText === 'Verificar'){
                      ...
                      navigation.navigate("Result", {puntuation3: puntaje,lesson:1,subtitle:'Colores/Animales'});
...
const styles = StyleSheet.create({
  ...
  receivingZone: {
    ...
    borderRadius: 10,
    width: 65,//(Dimensions.get('window').width / 4) - 12,
    justifyContent: 'center',
    alignItems: 'center',
    marginRight: 0.1
  },
